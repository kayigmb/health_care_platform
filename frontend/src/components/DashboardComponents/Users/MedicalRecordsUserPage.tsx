...
export function MedicalRecordsUserPage() {
  ...
  interface MedicalTable extends MedicalRecordType {
    actions?: React.ReactNode;
  }

  const fetchRecords = async () => {
    const res = await fetchData({ url: `${APIRoutesNames.MEDICAL_RECORDS}?patient=${user?.id}` });
    if (res) {
        ...
        <DialogContent dividers>
          {selectedRecord && (
            <Box display="flex" flexDirection="column" gap={2}>
              ...
              {selectedRecord.medicalDocumentsType && (
                <Box>
                  <Typography>
